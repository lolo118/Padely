...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let app, auth, db; // Define variables outside try block
    ...
    // --- Menú Móvil ---
    const toggleMenu = () => {
         if (sidebar) sidebar.classList.toggle('-translate-x-full'); // Check sidebar
         if (menuOverlay) menuOverlay.classList.toggle('hidden'); // Check overlay
    };
    ...
    if (navLinks.length > 0) {
        ...
        navLinks.forEach((link, index) => {
            link.addEventListener('click', (e) => {
                console.log(`Nav link ${index+1} (${link.dataset.page}) clicked.`); // <-- Log dentro del listener
                e.preventDefault();
    ...
    if (roleSelectionStep && formSteps) { // Check both exist
     ...
     } else { console.warn("Back to roles button not found in register modal."); } // Warn instead of error
    ...
    function showTournamentListView() {
         if(tournamentListPage) {
              console.log("Showing tournament list view, initializing charts..."); // <-- Log show list
             initializeCharts();
